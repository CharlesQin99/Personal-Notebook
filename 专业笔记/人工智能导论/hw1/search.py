...
def depthFirstSearch(problem):
    ...
    while not states.isEmpty():
        current, actions = states.pop()
        if problem.isGoalState(current):
            return actions

        if current not in record:
            exp = problem.expand(current)
            #print(exp)
            record.append(current)
            for location, direction, cost in exp:
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    s = problem.getStartState()
    states.push((s,[]), heuristic(s, problem))
    record = []
    while not states.isEmpty():
        current, actions = states.pop()
        if problem.isGoalState(current):
        ...
        if current not in record:
            record.append(current)
            exp = problem.expand(current)
            for location, action, cost in exp:
                tempActions = actions + [action]
                nextCost = problem.getCostOfActionSequence(tempActions) + heuristic(location, problem)
                if location not in record:
                    states.push((location,tempActions), nextCost)
    util.raiseNotDefined()
